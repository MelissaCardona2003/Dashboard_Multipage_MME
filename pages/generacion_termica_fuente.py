...
register_page(
    __name__,
    path="/generacion/termica/fuente",
    name="Fuente Térmica",
    title="Tablero Fuente Térmica - Ministerio de Minas y Energía de Colombia",
    order=7
)
...
if PYDATAXM_AVAILABLE:
    try:
        objetoAPI = ReadDB()
        print("✅ API XM inicializada correctamente en fuente térmica")
    except Exception as e:
        print(f"❌ Error al inicializar API XM en fuente térmica: {e}")
        objetoAPI = None

def obtener_listado_recursos():
    """Obtener el listado de recursos para identificar plantas térmicas"""
    try:
        ...
        if recursos is not None and not recursos.empty:
            # Filtrar solo plantas térmicas (Values_Type)
            if 'Values_Type' in recursos.columns:
                plantas_termicas = recursos[
                    recursos['Values_Type'].str.contains('TERMICA|CARBON|GAS|COMBUSTIBLE', na=False, case=False)
                ].copy()
                return plantas_termicas
            else:
                print("No se encontró la columna 'Values_Type' en recursos")
        print("No se obtuvieron recursos térmicos")
        return pd.DataFrame()
    except Exception as e:
        print(f"Error obteniendo listado de recursos térmicos: {e}")
    return pd.DataFrame()
...
def obtener_generacion_plantas(fecha_inicio, fecha_fin, plantas_df=None):
    """Obtener datos de generación por plantas térmicas (Gene por Recurso con SIC y lotes)"""
    try:
        ...
        print(f"Error en obtener_generacion_plantas térmicas: {e}")
        traceback.print_exc()
        return pd.DataFrame(), pd.DataFrame()

def crear_grafica_temporal_negra(df_generacion, planta_seleccionada=None):
    """Gráfica con línea negra: serie nacional (suma) y opcionalmente la planta seleccionada (línea negra punteada)."""
    ...
    fig.update_layout(
        title='Evolución Temporal de Generación Térmica',
...
def crear_tabla_participacion(df_participacion):
    ...
    leyenda = html.Div([
        ...
        html.Ul([
            ...
            html.Li("Nota: Los percentiles se recalculan al cambiar fechas o filtros. Los colores no implican calidad técnica ni disponibilidad; solo posición relativa en este período.")
    ...
    tabla = dash_table.DataTable(
        ...
        style_header={
            'backgroundColor': COLORS['energia_termica'],
            'color': 'white',
            'fontWeight': 'bold'
        },
...
layout = html.Div([
    ...
    # Header
    crear_header(
        titulo_pagina="Fuente Térmica",
        descripcion_pagina="Tablero de plantas térmicas con generación y participación del Sistema Interconectado Nacional",
    ...
    dbc.Container([
        ...
        html.Div([
            html.H2([
                ...
                "Tablero Fuente Térmica"
            ], className="mb-3", style={"color": COLORS['text_primary']}),
            html.P("Análisis detallado de generación por plantas térmicas del sistema eléctrico colombiano", 
        ...
        dcc.Loading(
            id="loading-termica",
            type="circle",
            children=html.Div(id="contenido-termica")
        ),
...
# Callbacks
@callback(
    [Output('planta-dropdown-termica', 'options'),
     Output('contenido-termica', 'children')],
    [Input('btn-actualizar-termica', 'n_clicks'),
     Input('date-range-termica', 'start_date'),
     Input('date-range-termica', 'end_date')],
    [State('planta-dropdown-termica', 'value')]
)
def actualizar_tablero_termica(n_clicks, fecha_inicio, fecha_fin, plantas_seleccionadas):
    ...
    try:
        ...
        # Obtener listado de plantas térmicas
        plantas_df = obtener_listado_recursos()
        
        if plantas_df.empty:
            return [], dbc.Alert("No se pudieron obtener los datos de plantas térmicas", color="warning")
            ...
            dbc.Card([
                dbc.CardHeader([
                    html.H5([
                        ...
                        "Evolución Temporal de Generación Térmica"
                    ], className="mb-0")
                ]),
                dbc.CardBody([
                    dcc.Graph(
            ...
            dbc.Card([
                dbc.CardHeader([
                    html.H5([
                        ...
                        "Participación por Planta Térmica"
                    ], className="mb-0")
                ]),
                dbc.CardBody([
        ...
        print(f"Error en callback térmica: {e}")
        traceback.print_exc()
