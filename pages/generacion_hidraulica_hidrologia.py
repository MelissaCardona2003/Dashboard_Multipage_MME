...
if _temp_api is not None:
    ...
    print("API XM no disponible (pydataxm)")
...
def crear_fichas_sin(fecha=None, region=None, rio=None):
    ...
    if rio and rio != "__ALL__":
        contexto = f"Río {rio}"
# REMOVED DEBUG:         print(f"🔍 [DEBUG] crear_fichas_sin llamada con fecha_calculo: {fecha_calculo} (RÍO: {rio})")
...
# REMOVED DEBUG:         print(f"🔍 [DEBUG] crear_fichas_sin llamada con fecha_calculo: {fecha_calculo} (REGIÓN: {region})")
...
# REMOVED DEBUG:         print(f"🔍 [DEBUG] crear_fichas_sin llamada con fecha_calculo: {fecha_calculo} (SIN COMPLETO)")
...
# REMOVED DEBUG:                 print(f"✅ SEMÁFORO CORREGIDO: {embalse_name} - Participación={participacion_val}%, Volumen={volumen_val}%")
                
                # Clasificar riesgo con ambos valores CORRECTOS
                nivel_riesgo = clasificar_riesgo_embalse(participacion_val, volumen_val)
# REMOVED DEBUG:                 print(f"✅ RESULTADO SEMÁFORO: {embalse_name} - Riesgo: {nivel_riesgo}")
...
# REMOVED DEBUG:         print(f"✅ DEBUG STORES: Vistas construidas exitosamente")
...
# REMOVED DEBUG:         print(f"✅ DataFrame de embalses creado con {len(df_embalses)} embalses")
...
# REMOVED DEBUG:             print(f"✅ Tabla de regiones creada con {len(regiones_totales)} regiones (usando función unificada)")
        else:
...
# REMOVED DEBUG:         print(f"✅ DATOS TABLA PREPARADOS: {len(table_data)} filas con columna de riesgo")
        return table_data
...
def get_embalses_capacidad(region=None, start_date=None, end_date=None):
    ...
    try:
        objetoAPI = get_objetoAPI()
# REMOVED DEBUG:         print(f"🔍 DEBUG CAPACIDAD: Iniciando con región={region}, fechas={start_date} a {end_date}")
...
# REMOVED DEBUG:             print(f"🔍 DEBUG CAPACIDAD: Mapeado región-embalse: {len(embalse_region_dict)} embalses")
# REMOVED DEBUG:             print(f"🔍 DEBUG CAPACIDAD: Regiones disponibles: {set(embalse_region_dict.values())}")
            if region:
                embalses_en_region = [e for e, r in embalse_region_dict.items() if r == region]
# REMOVED DEBUG:                 print(f"🔍 DEBUG CAPACIDAD: Embalses en región {region}: {embalses_en_region}")
...
# REMOVED DEBUG:             print(f"🔍 DEBUG CAPACIDAD: Embalses con datos de capacidad: {len(embalses_con_datos)}")
# REMOVED DEBUG:             print(f"🔍 DEBUG CAPACIDAD: Embalses filtrados: {len(embalse_region_dict_filtrado)}")
...
# REMOVED DEBUG:                 print(f"🔍 DEBUG CAPACIDAD: Filtro por región '{region}' -> '{region_normalized}' - antes: {antes_filtro}, después: {len(df_capacidad)}")
...
# REMOVED DEBUG:             print(f"📊 Columnas finales: {list(df_final.columns)}")
# REMOVED DEBUG:             print(f"📊 Primeras filas del DataFrame final:")
            print(df_final.head())

# REMOVED DEBUG:             print(f"🔍 DEBUG CAPACIDAD: Retornando DataFrame con {len(df_final)} filas")
...
# REMOVED DEBUG:                 print(f"✅ Agregada columna: Embalse")
            elif col == "Volumen Útil (%)":
                columns.append({"name": "Volumen Útil (%)", "id": "Volumen Útil (%)"})
# REMOVED DEBUG:                 print(f"✅ Agregada columna: Volumen Útil (%)")
            elif col == "Participación (%)":
                columns.append({"name": "Participación (%)", "id": "Participación (%)"})
# REMOVED DEBUG:                 print(f"✅ Agregada columna: Participación (%)")
            elif col == "Riesgo":
                columns.append({"name": "🚨 Riesgo", "id": "Riesgo"})
# REMOVED DEBUG:                 print(f"✅ Agregada columna: Riesgo")
...
# REMOVED DEBUG:         print(f"📊 Datos iniciales obtenidos: {df.shape[0]} filas, columnas: {list(df.columns)}")
...
# REMOVED DEBUG:         print(f"📊 DataFrame final para tabla inicial: {df_final_display.shape[0]} filas, columnas: {list(df_final_display.columns)}")
...
# REMOVED DEBUG:     print(f"✅ Tabla DataTable creada exitosamente con ID: {table.id}")
    return table
...
def get_porcapor_data(fecha_inicio, fecha_fin):
    ...
    try:
        objetoAPI = get_objetoAPI()
# REMOVED DEBUG:         print(f"🔍 Consultando PorcApor desde {fecha_inicio} hasta {fecha_fin}")
        data = objetoAPI.request_data('PorcApor', 'Rio', fecha_inicio, fecha_fin)
        if not data.empty:
            # Multiplicar por 100 para convertir a porcentaje
            if 'Value' in data.columns:
                data['Value'] = data['Value'] * 100
# REMOVED DEBUG:             print(f"✅ Datos PorcApor obtenidos: {len(data)} registros")
            return data
        else:
...
# REMOVED DEBUG:         print(f"🔍 DEBUG: Primeras filas de df_date: {df_date.head(3).to_dict() if not df_date.empty else 'No hay datos'}")
...
# REMOVED DEBUG:         print(f"✅ DEBUG: Retornando modal abierto con tabla de {len(data_with_total)} filas")
...
# REMOVED DEBUG:     print(f"⚠️ DEBUG: No se detectó ningún click válido - modal cerrado por defecto")
    return False, None, "", ""
...
def create_region_filtered_participacion_table(region, start_date, end_date):
    ...
    """
    try:
# REMOVED DEBUG:         print(f"🔍 DEBUG PARTICIPACIÓN: Iniciando para región={region}, fechas={start_date} a {end_date}")
        
        # Obtener datos de embalses filtrados por región
        df_embalses = get_embalses_capacidad(region, start_date, end_date)
# REMOVED DEBUG:         print(f"🔍 DEBUG PARTICIPACIÓN: get_embalses_capacidad retornó {len(df_embalses)} filas")
# REMOVED DEBUG:         print(f"🔍 DEBUG PARTICIPACIÓN: Embalses encontrados: {df_embalses['Embalse'].tolist() if not df_embalses.empty else 'NINGUNO'}")
...
# REMOVED DEBUG:         print(f"🔍 DEBUG PARTICIPACIÓN: get_participacion_embalses retornó {len(df_participacion)} filas")
...
# REMOVED DEBUG:             print(f"✅ SEMÁFORO REGIÓN {region}: {embalse_name} - Participación={participacion_num}%, Volumen={volumen_util}%")
# REMOVED DEBUG:             print(f"✅ RESULTADO SEMÁFORO REGIÓN {region}: {embalse_name} - Riesgo: {nivel_riesgo}")
...
# REMOVED DEBUG:         print(f"🔍 DEBUG PARTICIPACIÓN: Datos finales de tabla: {len(table_data)} filas")
...
# REMOVED DEBUG:         print(f"❌ Error en create_region_filtered_participacion_table: {e}")
...
# REMOVED DEBUG:             print(f"✅ SEMÁFORO CAPACIDAD REGIÓN {region}: {embalse_name} - Participación={participacion_num}%, Volumen={volumen_util}%")
# REMOVED DEBUG:             print(f"✅ RESULTADO CAPACIDAD REGIÓN {region}: {embalse_name} - Riesgo: {nivel_riesgo}")
...
# REMOVED DEBUG:         print(f"❌ Error en create_region_filtered_capacidad_table: {e}")
        return html.Div("Error al cargar los datos.", className="text-center text-danger")
