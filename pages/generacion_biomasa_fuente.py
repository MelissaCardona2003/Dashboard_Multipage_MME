...
register_page(
    __name__,
    path="/generacion/biomasa/fuente",
    name="Fuente Biomasa",
    title="Tablero Fuente Biomasa - Ministerio de Minas y Energía de Colombia",
    order=11
)
...
if PYDATAXM_AVAILABLE:
    try:
        objetoAPI = ReadDB()
        print("✅ API XM inicializada correctamente en fuente biomasa")
    except Exception as e:
        print(f"❌ Error al inicializar API XM en fuente biomasa: {e}")
        objetoAPI = None

def obtener_listado_recursos():
    """Obtener listado de recursos de biomasa desde ListadoRecursos.
    Intenta filtrar por múltiples columnas candidatas (Values_Type, Values_Tech, Values_EnerSource, Values_Fuel, etc.).
    """
    try:
        if objetoAPI is None:
            return pd.DataFrame()
        # Usar ventana similar a otras fuentes para alta probabilidad de datos
        fecha_fin = date.today() - timedelta(days=14)
        fecha_ini = fecha_fin - timedelta(days=7)
        recursos = objetoAPI.request_data("ListadoRecursos", "Sistema", fecha_ini, fecha_fin)
        if recursos is None or recursos.empty:
            return pd.DataFrame()
        # Columnas candidatas donde puede venir el tipo/fuente
        columnas_candidatas = [
            'Values_Type', 'Values_Tech', 'Values_EnerSource', 'Values_Fuel',
            'Values_Source', 'Values_Category', 'Values_Classification'
        ]
        presentes = [c for c in columnas_candidatas if c in recursos.columns and recursos[c].dtype == 'object']
        if not presentes:
            # Fallback: si no hay columnas de tipo conocidas, retornar lo que haya
            return recursos.copy()
        patrones = r"BIOM|BIOG|BAGAZO|RESID|LIXIV|ORGANI|BIOENER|BIO-MASA|BIO GAS|BIOGAS"
        mask = pd.Series(False, index=recursos.index)
        for col in presentes:
            mask = mask | recursos[col].astype(str).str.contains(patrones, na=False, case=False)
        filtrados = recursos[mask].copy()
        # Quitar duplicados por código si existe, si no por nombre
        if 'Values_Code' in filtrados.columns:
            filtrados = filtrados.drop_duplicates(subset=['Values_Code'])
        elif 'Values_Name' in filtrados.columns:
            filtrados = filtrados.drop_duplicates(subset=['Values_Name'])
        return filtrados
    except Exception as e:
        print(f"Error obteniendo listado de recursos biomasa: {e}")
        return pd.DataFrame()
...
def obtener_generacion_plantas(fecha_inicio, fecha_fin, plantas_df=None):
    """Obtener datos de generación por plantas de biomasa usando Gene por Recurso (SIC) con lotes+chunking."""
    try:
        ...
        participacion_total['Estado'] = participacion_total['Participacion_%'].apply(lambda x: 'Alto' if x>=20 else ('Medio' if x>=5 else 'Bajo'))
        return df_generacion, participacion_total
    except Exception as e:
        print(f"Error en obtener_generacion_plantas biomasa: {e}")
        traceback.print_exc()
        return pd.DataFrame(), pd.DataFrame()
...
def crear_grafica_temporal_negra(df_generacion, planta_seleccionada=None):
    ...
    fig.update_layout(
        title='Evolución Temporal de Generación con Biomasa',
...
def crear_tabla_participacion(df_participacion):
    ...
    # Umbrales adaptativos por percentiles de participación
    ...
    if len(dfp) >= 3 and dfp['Participacion_%'].sum() > 0:
        ...
        # Fallback razonable si hay pocos datos
        p50, p75 = 10.0, 25.0
    ...
    leyenda = html.Div([
        ...
        html.Ul([
            ...
            html.Li("Nota: Los percentiles se recalculan al cambiar el rango de fechas o filtros. Los colores no evalúan calidad técnica ni disponibilidad; muestran posición relativa en el período seleccionado.")
    ...
    tabla = dash_table.DataTable(
        ...
        style_header={
            'backgroundColor': COLORS['energia_biomasa'],
            'color': 'white',
            'fontWeight': 'bold'
        },
...
layout = html.Div([
    ...
    # Header
    crear_header(
        titulo_pagina="Fuente Biomasa",
        descripcion_pagina="Tablero de plantas de biomasa con generación y participación del Sistema Interconectado Nacional",
    ...
    dbc.Container([
        ...
        html.Div([
            html.H2([
                ...
                "Tablero Fuente Biomasa"
            ], className="mb-3", style={"color": COLORS['text_primary']}),
            html.P("Análisis detallado de generación por plantas de biomasa del sistema eléctrico colombiano", 
        ...
        dcc.Loading(
            id="loading-biomasa",
            type="circle",
            children=html.Div(id="contenido-biomasa")
        ),
...
# Callbacks
@callback(
    [Output('planta-dropdown-biomasa', 'options'),
    Output('contenido-biomasa', 'children')],
    [Input('btn-actualizar-biomasa', 'n_clicks'),
     Input('date-range-biomasa', 'start_date'),
     Input('date-range-biomasa', 'end_date')],
    [State('planta-dropdown-biomasa', 'value')]
)
def actualizar_tablero_biomasa(n_clicks, fecha_inicio, fecha_fin, plantas_seleccionadas):
    # Cargar automáticamente en primer render y cuando cambien fechas
    ...
    try:
        ...
        # Obtener listado de plantas de biomasa
        plantas_df = obtener_listado_recursos()
        
        if plantas_df.empty:
            return [], dbc.Alert("No se pudieron obtener los datos de plantas de biomasa", color="warning")
        
        # Opciones para el dropdown usando SIC
        col_sic = _detectar_columna_sic(plantas_df, fecha_inicio_dt, fecha_fin_dt)
        opciones_plantas = []
        if col_sic:
            # Etiqueta: Nombre (SIC) pero selección única
            opciones_plantas = [
                {'label': f"{row.get('Values_Name', 'Sin nombre')} ({row[col_sic]})", 'value': row[col_sic]}
        ...
        card_diag = dbc.Card([
            ...
            dbc.CardBody([
                html.Ul([
                    html.Li(f"Recursos biomasa detectados: {len(plantas_df)}"),
                    html.Li(f"Columna SIC detectada: {col_sic or 'No detectada'}"),
                    html.Li(f"Ejemplos de SIC: {', '.join(ejemplos_cod) if ejemplos_cod else 'N/D'}"),
                    html.Li(f"Rango consultado Gene: {fecha_inicio_dt} a {fecha_fin_dt}")
                ], className="mb-0")
            ])
        ], className="mb-3")
        ...
        if df_generacion.empty:
            aviso = dbc.Alert(
                "No se encontraron datos de generación para el período seleccionado. Prueba ampliando el rango (p. ej., 60-90 días).",
                color="info"
            )
            return opciones_plantas, html.Div([card_diag, aviso])
            ...
            dbc.Card([
                dbc.CardHeader([
                    html.H5([
                        ...
                        "Evolución Temporal de Generación con Biomasa"
                    ], className="mb-0")
                ]),
                dbc.CardBody([
                    dcc.Graph(
            ...
            dbc.Card([
                dbc.CardHeader([
                    html.H5([
                        ...
                        "Participación por Planta de Biomasa"
                    ], className="mb-0")
                ]),
                dbc.CardBody([
        ...
        print(f"Error en callback biomasa: {e}")
        traceback.print_exc()
