...
def obtener_listado_recursos():
    ...
    try:
        ...
        if recursos is not None and not recursos.empty:
            # Filtrar solo plantas eólicas (Values_Type)
            if 'Values_Type' in recursos.columns:
                plantas_eolicas = recursos[
                    recursos['Values_Type'].str.contains('EOLICA', na=False, case=False)
                ].copy()
                return plantas_eolicas
            else:
                print("No se encontró la columna 'Values_Type' en recursos")
        print("No se obtuvieron recursos eólicos")
        return pd.DataFrame()
    except Exception as e:
...
def obtener_generacion_plantas(fecha_inicio, fecha_fin, plantas_df=None):
    """Obtener datos de generación por plantas eólicas (Gene por Recurso con SIC y lotes)"""
    try:
        ...
        participacion_total['Estado'] = participacion_total['Participacion_%'].apply(lambda x: 'Alto' if x>=15 else ('Medio' if x>=5 else 'Bajo'))
...
def crear_grafica_temporal_negra(df_generacion, planta_seleccionada=None):
    """Gráfica con línea negra: serie nacional (suma) y opcionalmente una sola planta seleccionada.
    Ambas en negro; se diferencia la planta por trazo punteado.
    """
...
def crear_tabla_participacion(df_participacion):
    ...
    leyenda = html.Div([
        ...
        html.Ul([
            ...
            html.Li("Nota: Los percentiles se recalculan cuando cambias fechas o filtros. Los colores no evalúan calidad técnica ni disponibilidad; muestran posición relativa en este período.")
...
layout = html.Div([
    ...
    dbc.Container([
        ...
        dcc.Loading(
            id="loading-eolica",
            type="circle",
            children=html.Div(id="contenido-eolica")
        ),
...
@callback(
    ...
    [Input('btn-actualizar-eolica', 'n_clicks'),
     Input('date-range-eolica', 'start_date'),
     Input('date-range-eolica', 'end_date')],
...
def actualizar_tablero_eolica(n_clicks, fecha_inicio, fecha_fin, plantas_seleccionadas):
    ...
    try:
        ...
        # Determinar planta seleccionada (una sola)
        planta_seleccionada = None
        if plantas_seleccionadas:
            # Si viene el código SIC, mapearlo a nombre de planta
