...
def get_reservas_hidricas(fecha):
    ...
    Fórmula: (VolumenUtilDiarioEnergia / CapacidadUtilSIN) * 100
    ...
    if not objetoAPI:
        print("[HIDROLOGIA] ❌ API XM no disponible - no se pueden obtener Reservas Hídricas")
        return None, None
    ...
    try:
        ...
        if volumen_util is not None and not volumen_util.empty and capacidad_util is not None and not capacidad_util.empty:
            volumen_valor = volumen_util['Value'].iloc[0]
            capacidad_valor = capacidad_util['Value'].iloc[0]
            
            print(f"[DEBUG] Volumen: {volumen_valor}, Capacidad: {capacidad_valor}")
            
            if capacidad_valor > 0:
                porcentaje = round((volumen_valor / capacidad_valor) * 100, 2)
                print(f"[DEBUG] Reservas calculadas: {porcentaje}%")
...
def get_aportes_hidricos(fecha):
    ...
    Fórmula: (AportesDiariosEnergia / MediaHistoricaAportesSIN) * 100
    ...
    try:
        ...
        if aportes_diarios is not None and not aportes_diarios.empty and media_historica is not None and not media_historica.empty:
            ...
            if media_valor > 0:
                ...
                print(f"[DEBUG] Aportes calculados: {porcentaje}%")
...
def get_reservas_hidricas_por_region(fecha, region):
    ...
    if not objetoAPI:
        print("[HIDROLOGIA] ❌ API XM no disponible - no se pueden obtener Reservas Hídricas por región")
        return None, None
    
    try:
        # Obtener datos de embalses de la región específica
        embalses_data = objetoAPI.request_data('VoluUtilDiarEner', 'Embalse', fecha, fecha)
        
        if embalses_data is not None and not embalses_data.empty:
            # Filtrar embalses que pertenecen a ríos de la región especificada
            embalses_data['Region'] = embalses_data['Name'].apply(
                lambda embalse: next((reg for rio, reg in RIO_REGION.items() 
                                    if any(word in embalse.upper() for word in rio.split())), None)
            )
            
            # Filtrar por región
            embalses_region = embalses_data[embalses_data['Region'] == region]
            
            if not embalses_region.empty:
                # Sumar volumen útil de todos los embalses de la región
                volumen_total_region = embalses_region['Value'].sum()
                
                # Obtener capacidad total de la región
                capacidad_data = objetoAPI.request_data('CapaUtilDiarEner', 'Embalse', fecha, fecha)
                if capacidad_data is not None and not capacidad_data.empty:
                    capacidad_data['Region'] = capacidad_data['Name'].apply(
                        lambda embalse: next((reg for rio, reg in RIO_REGION.items() 
                                            if any(word in embalse.upper() for word in rio.split())), None)
                    )
                    
                    capacidad_region = capacidad_data[capacidad_data['Region'] == region]
                    if not capacidad_region.empty:
                        capacidad_total_region = capacidad_region['Value'].sum()
                        
                        if capacidad_total_region > 0:
                            porcentaje = round((volumen_total_region / capacidad_total_region) * 100, 2)
                            print(f"[DEBUG] Reservas región {region}: {porcentaje}%")
                            return porcentaje, volumen_total_region
        
        print(f"[DEBUG] Sin datos suficientes para calcular reservas de región {region}")
        return None, None
        
    except Exception as e:
        print(f"[HIDROLOGIA] Error obteniendo reservas hídricas por región: {e}")
        return None, None
...
def get_aportes_hidricos_por_region(fecha, region):
    ...
    try:
        ...
        if aportes_data is not None and not aportes_data.empty:
            ...
            # Filtrar por región
            aportes_region = aportes_data[aportes_data['Region'] == region]
            
            if not aportes_region.empty:
                # Sumar aportes de todos los ríos de la región
                aportes_total_region = aportes_region['Value'].sum()
                
                # Para el porcentaje, usar la media histórica regional
                # (esto es una aproximación, idealmente necesitaríamos la media histórica específica por región)
                media_historica_data = objetoAPI.request_data('AporEnerMediHist', 'Sistema', fecha, fecha)
                
                if media_historica_data is not None and not media_historica_data.empty:
                    media_historica_sin = media_historica_data['Value'].iloc[0]
                    
                    # Calcular proporción aproximada de la región respecto al SIN
                    # (esto es una aproximación basada en la contribución actual)
                    aportes_sin_total = objetoAPI.request_data('AporCaudal', 'Rio', fecha, fecha)
                    if aportes_sin_total is not None and not aportes_sin_total.empty:
                        total_sin = aportes_sin_total['Value'].sum()
                        if total_sin > 0:
                            proporcion_region = aportes_total_region / total_sin
                            media_historica_region = media_historica_sin * proporcion_region
                            
                            if media_historica_region > 0:
                                porcentaje = round((aportes_total_region / media_historica_region) * 100, 2)
...
def crear_fichas_sin(fecha=None, region=None, rio=None):
    ...
    """
    # Usar fecha proporcionada o fecha de ayer como fallback (más probable que tenga datos)
    fecha_calculo = fecha if fecha else (date.today() - timedelta(days=1)).strftime('%Y-%m-%d')
    
    # Determinar el contexto de los cálculos
    if rio and rio != "__ALL__":
        contexto = f"Río {rio}"
        print(f"🔍 [DEBUG] crear_fichas_sin llamada con fecha_calculo: {fecha_calculo} (RÍO: {rio})")
        
        # Para río específico, solo calculamos aportes (no hay reservas por río individual)
        ...
        # Obtener datos filtrados por región
        ...
        # Obtener datos del SIN completo (funciones originales)
...
def render_hidro_tab_content(active_tab):
    if active_tab == "tab-consulta":
        return html.Div([
            ...
            # Loading indicator con mensaje personalizado
            dcc.Loading(
                id="loading-hidro",
                type="circle",
                children=html.Div([
                    # Fichas KPI del SIN que se muestran inicialmente
                    html.Div(id="fichas-kpi-container", children=crear_fichas_sin_seguras()),
                    html.Div(id="hidro-results-content-dynamic", className="mt-4")
...
# Nuevo callback para actualizar las fichas KPI cuando cambian los filtros
@callback(
...
def update_fichas_kpi(region, rio, start_date):
    """
    Actualiza las fichas KPI según los filtros seleccionados
    """
    print(f"🔄 [DEBUG] Actualizando fichas KPI: región={region}, río={rio}, fecha={start_date}")
    
    # Usar la fecha seleccionada o fecha de ayer como fallback
    fecha_calculo = start_date if start_date else (date.today() - timedelta(days=1)).strftime('%Y-%m-%d')
    ...
    # Crear las fichas con los filtros aplicados
    return crear_fichas_sin(fecha=fecha_calculo, region=region_filtro, rio=rio_filtro)
...
def update_content(n_clicks, rio, start_date, end_date, region):
    ...
    try:
        ...
        if region and region != "__ALL_REGIONS__":
            ...
            embalses_df_fresh = get_embalses_capacidad(region, start_date, end_date)
        ...
        if 'Name' in data_filtered.columns and 'Value' in data_filtered.columns:
            ...
            if region and region != "__ALL_REGIONS__":
                ...
                region_temporal_data['Region'] = region
                
                return html.Div([
                    html.H5(f"🏞️ Evolución Temporal de Aportes de Caudal - Región {region}", className="text-center mb-2"),
                    html.P(f"Serie temporal de aportes de caudal para la región {region}. La gráfica muestra la evolución diaria de los aportes de caudal de todos los ríos de esta región durante el período seleccionado.", className="text-center text-muted mb-3", style={"fontSize": "0.9rem"}),
...
def initialize_hierarchical_tables(start_date, end_date):
    ...
    try:
        ...
        for _, row in regiones_totales.iterrows():
            ...
            # Datos de capacidad -> cambiar a volumen útil
            volumen_util_valor = row.get('Volumen Útil Promedio (%)', 0)
        ...
        # Calcular promedio general de volumen útil para TOTAL
        promedio_volumen_general = regiones_totales['Volumen Útil Promedio (%)'].mean() if not regiones_totales.empty else 0
        capacidad_data.append({
            'nombre': 'TOTAL SISTEMA',
...
def get_tabla_regiones_embalses(start_date=None, end_date=None):
    ...
    try:
        ...
        embalses_capacidad = get_embalses_capacidad(None, start_date, end_date)
        ...
        # Agrupar por región para capacidad
        regiones_totales = df_completo.groupby('Values_HydroRegion')['Capacidad_GWh_Internal'].sum().reset_index()
        regiones_totales = regiones_totales.rename(columns={'Values_HydroRegion': 'Región', 'Capacidad_GWh_Internal': 'Total (GWh)'})
        regiones_totales = regiones_totales.sort_values('Total (GWh)', ascending=False)
        
        # Agrupar por región para volumen útil (promedio)
        if 'Volumen Útil (%)' in df_completo.columns:
            # Filtrar datos válidos de volumen útil
            df_volumen_valido = df_completo[df_completo['Volumen Útil (%)'].notna()]
            if not df_volumen_valido.empty:
                regiones_volumen = df_volumen_valido.groupby('Values_HydroRegion')['Volumen Útil (%)'].mean().reset_index()
                regiones_volumen = regiones_volumen.rename(columns={'Values_HydroRegion': 'Región', 'Volumen Útil (%)': 'Volumen Útil Promedio (%)'})
                # Combinar con los datos de capacidad
                regiones_totales = regiones_totales.merge(regiones_volumen, on='Región', how='left')
            else:
                regiones_totales['Volumen Útil Promedio (%)'] = 0
        else:
            regiones_totales['Volumen Útil Promedio (%)'] = 0
...
def get_embalses_capacidad(region=None, start_date=None, end_date=None):
    ...
    y el porcentaje de volumen útil diario (PorcVoluUtilDiar).
    ...
    try:
        ...
        # Obtener datos de capacidad útil diaria de energía
        df_capacidad = objetoAPI.request_data('CapaUtilDiarEner','Embalse', start_date, end_date)
        print(f"🔍 DEBUG CAPACIDAD: Datos de capacidad obtenidos: {len(df_capacidad)} registros")
        
        # Obtener datos de porcentaje de volumen útil diario
        df_porcentaje = objetoAPI.request_data('PorcVoluUtilDiar','Embalse', start_date, end_date)
        print(f"📊 Datos de capacidad útil: {len(df_capacidad)} registros")
        print(f"💧 Datos de porcentaje de volumen útil: {len(df_porcentaje)} registros")
        
        # DEBUG: Verificar datos de capacidad
        if not df_capacidad.empty:
            print(f"🔍 Capacidad total registrada: {df_capacidad['Value'].sum():,.0f} Wh")
            print(f"🔍 Embalses en datos de capacidad: {df_capacidad['Name'].unique()[:10]}")
        
        if 'Name' in df_capacidad.columns and 'Value' in df_capacidad.columns:
            # Obtener información de región para embalses
            # Usar las mismas fechas para consistencia
            today = datetime.now().strftime('%Y-%m-%d')
            yesterday = (datetime.now() - timedelta(days=1)).strftime('%Y-%m-%d')
            embalses_info = objetoAPI.request_data('ListadoEmbalses','Sistema', yesterday, today)
            embalses_info['Values_Name'] = embalses_info['Values_Name'].str.strip().str.upper()
            ...
            if region:
                ...
                df_capacidad = df_capacidad[df_capacidad['Region'] == region]
                print(f"🔍 DEBUG CAPACIDAD: Filtro por región {region} - antes: {antes_filtro}, después: {len(df_capacidad)}")
            ...
            # Procesar datos de porcentaje de volumen útil
            df_porcentaje_final = pd.DataFrame()
            if not df_porcentaje.empty and 'Name' in df_porcentaje.columns and 'Value' in df_porcentaje.columns:
                df_porcentaje['Region'] = df_porcentaje['Name'].map(embalse_region_dict_filtrado)
                if region:
                    antes_filtro_pct = len(df_porcentaje)
                    df_porcentaje = df_porcentaje[df_porcentaje['Region'] == region]
                    print(f"🔍 DEBUG PORCENTAJE: Filtro por región {region} - antes: {antes_filtro_pct}, después: {len(df_porcentaje)}")
                # Multiplicar por 100 para convertir a porcentaje
                df_porcentaje['Value'] = df_porcentaje['Value'] * 100
                df_porcentaje_grouped = df_porcentaje.groupby('Name')['Value'].mean().reset_index()
                df_porcentaje_grouped = df_porcentaje_grouped.rename(columns={'Name': 'Embalse', 'Value': 'Volumen Útil (%)'})
                df_porcentaje_final = df_porcentaje_grouped
            
            # Combinar ambos datasets
            if not df_porcentaje_final.empty:
                df_final = pd.merge(df_capacidad_grouped, df_porcentaje_final, on='Embalse', how='left')
                print(f"📊 Merge exitoso - Columnas finales: {list(df_final.columns)}")
                print(f"📊 Primeras filas del DataFrame final:")
                print(df_final.head())
            else:
                df_final = df_capacidad_grouped.copy()
                df_final['Volumen Útil (%)'] = None
                print("⚠️ No hay datos de porcentaje - columna creada con valores None")
            ...
            return pd.DataFrame(columns=['Embalse', 'Volumen Útil (%)'])
    except Exception as e:
