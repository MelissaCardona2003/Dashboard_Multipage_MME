...
def configurar_logging():
    ...
    if not logger.handlers:
        handler = logging.FileHandler('logs/tablero_hidraulica.log')
...
try:
    ...
    logger.warning("pydataxm no disponible")

objetoAPI = None
if PYDATAXM_AVAILABLE:
    try:
        objetoAPI = ReadDB()
        logger.info("API XM inicializada")
    except Exception as e:
        logger.error(f"Error inicializando API XM: {e}")
        objetoAPI = None

register_page(__name__, path="/generacion/hidraulica/fuente", name="Fuente Hidráulica", title="Fuente Hidráulica - Ministerio de Minas y Energía de Colombia", order=1)
    ...
    try:
        ...
        if recursos is not None and not recursos.empty:
            if 'Values_Type' in recursos.columns:
                return recursos[recursos['Values_Type'].str.contains('HIDRAULICA', na=False, case=False)].copy()
        return pd.DataFrame()
    except Exception as e:
        logger.error(f"Error obteniendo listado recursos: {e}")
        return pd.DataFrame()
...
def _detectar_columna_sic(recursos_df: pd.DataFrame, f_ini: date, f_fin: date):
    ...
    for col in orden:
        ...
        try:
            ...
            logger.warning(f"Candidata {col} falló: {e}")
            continue
    logger.error("No fue posible detectar columna SIC")
    return None
...
def obtener_generacion_plantas(fecha_inicio, fecha_fin, plantas_df=None):
    try:
        ...
        if not codigos:
            return pd.DataFrame(), pd.DataFrame()
        df_gen = fetch_gene_recurso_chunked(objetoAPI, fecha_inicio, fecha_fin, codigos, batch_size=50, chunk_days=30)
        ...
        df_gen = df_gen.merge(plantas_min, on='Codigo', how='left')
        part = df_gen.groupby('Planta', as_index=False)['Generacion_GWh'].sum()
        total = part['Generacion_GWh'].sum()
        part['Participacion_%'] = (part['Generacion_GWh']/total*100).round(2) if total>0 else 0.0
        part['Estado'] = part['Participacion_%'].apply(lambda x: 'Alto' if x>=20 else ('Medio' if x>=5 else 'Bajo'))
        return df_gen, part
    except Exception as e:
        logger.error(f"Error en obtener_generacion_plantas: {e}")
        traceback.print_exc()
        return pd.DataFrame(), pd.DataFrame()

def crear_grafica_temporal_negra(df_generacion, planta_seleccionada=None):
    """Gráfica minimalista: línea negra para la serie nacional y, opcionalmente, línea negra punteada para una planta seleccionada."""
    if df_generacion.empty:
        return go.Figure().add_annotation(text="No hay datos disponibles",
                                          xref="paper", yref="paper", x=0.5, y=0.5)

    # Serie nacional (suma por día)
...
def crear_tabla_participacion(df_participacion):
    ...
    # Umbrales adaptativos por percentiles sobre la participación (%)
    ...
    if len(dfp) >= 3 and dfp['Participacion_%'].sum() > 0:
        ...
        # Fallback mínimo cuando hay pocas plantas o datos planos
        p50, p75 = 5.0, 20.0
    # Recalcular estado con percentiles
    def clas(v):
    ...
    # Leyenda visible del semáforo (explicativa para usuarios no técnicos)
    leyenda = html.Div([
        ...
        html.Ul([
            ...
            html.Li("Nota: Los percentiles se recalculan cuando cambias el rango de fechas o filtros. Los colores no juzgan calidad técnica ni disponibilidad; reflejan posición relativa en el período seleccionado.")
...
layout = html.Div([
    ...
    dbc.Container([
        ...
        dcc.Loading(
            ...
            type="circle",
            children=html.Div(id="contenido-hidraulica")
        ),
...
@callback(
    ...
    [Input('btn-actualizar-hidraulica', 'n_clicks'),
     Input('date-range-hidraulica', 'start_date'),
     Input('date-range-hidraulica', 'end_date')],
...
def actualizar_tablero_hidraulica(n_clicks, fecha_inicio, fecha_fin, plantas_seleccionadas):
    # Cargar automáticamente en el primer render y cuando cambien fechas
