...
for metrica in metricas_gwh:
    ...
    SELECT 
        ...
        COUNT(*) as registros
    FROM metricas_energeticas
    WHERE MetricID = '{metrica}'
    """
    ...
    if not df.empty and df['registros'][0] > 0:
        ...
        print(f"  {metrica:25s}: Min={min_val:12.2f} | Max={max_val:12.2f} | Avg={avg_val:10.2f} | N={df['registros'][0]:6,}{sospechoso}")
...
SELECT 
    COUNT(*) as total,
    SUM(CASE WHEN Value > 1000000 THEN 1 ELSE 0 END) as valores_sospechosos,
    MAX(Value) as max_value
FROM metricas_energeticas
WHERE MetricID = 'AporEnerMediHist'
"""
...
SELECT 
    MetricID,
    COUNT(*) as total_registros,
    SUM(CASE WHEN Value > 1000000 THEN 1 ELSE 0 END) as valores_muy_grandes,
    SUM(CASE WHEN Value < 0 THEN 1 ELSE 0 END) as valores_negativos,
    MAX(Value) as max_value,
...
if not metricas_sospechosas.empty:
    ...
    for idx, row in metricas_sospechosas.iterrows():
        print(f"  - {row['MetricID']:30s}: {row['valores_muy_grandes']:5,} valores >1M | {row['valores_negativos']:5,} negativos | Max={row['max_value']:,.0f}")
else:
...
for idx, row in recientes.iterrows():
    ...
    print(f"{actualizado} {row['MetricID']:30s}: {row['registros_30dias']:5,} registros | Último: {row['ultima_fecha']} ({dias_desde_ultimo}d)")
...
for metric_id, descripcion in metricas_criticas.items():
    ...
    SELECT 
        ...
        MIN(Date) as desde,
        MAX(Date) as hasta,
        COUNT(DISTINCT Name) as entidades
    FROM metricas_energeticas
...
# 8. REVISAR CONFIGURACIÓN DEL ETL
print("\n⚙️  8. REVISIÓN DE CONFIGURACIÓN DEL ETL")
print("-"*80)
